...
pub const Type = union(enum) {
    ...
    // Profile enumeration for semantic analysis
    ...
    // Function call record for instrumentation
...
pub const Symbol = struct {
    ...
    type: Type,
    kind: SymbolKind,
    /// Registry of function call records for instrumentation
    function_calls: std.ArrayList(Type.FunctionCallRecord),
    /// Current active profile for semantic analysis
    active_profile: Type.Profile,
...
/// Foreign graft declaration captured from use/graft directives
pub const ForeignGraft = struct {
    /// Optional alias when using `graft <alias> = zig "module";`
    alias: ?astdb.StrId,
    /// Origin runtime identifier (e.g., zig, c, python) as interned id
    origin: astdb.StrId,
    /// Module name (string literal, dequoted) stored as allocated bytes
    module: []u8,
};
...
pub const SemanticGraph = struct {
    ...
    /// Registered foreign graft modules discovered from use/graft directives
    foreign_grafts: std.ArrayList(ForeignGraft),
    /// Function call records for instrumentation
    function_calls: std.ArrayList(Type.FunctionCallRecord),
    /// Current active profile
    active_profile: Type.Profile,

    pub fn init(allocator: std.mem.Allocator, astdb_system: *astdb.ASTDBSystem, profile: Type.Profile) !SemanticGraph {
        const snapshot = try astdb_system.createSnapshot();
        return SemanticGraph{
            .symbols = try std.ArrayList(Symbol).initCapacity(allocator, 0),
            .required_capabilities = try std.ArrayList(Type).initCapacity(allocator, 0),
            ...
            .function_calls = try std.ArrayList(Type.FunctionCallRecord).initCapacity(allocator, 0),
            ...
            .foreign_grafts = try std.ArrayList(ForeignGraft).initCapacity(allocator, 0),
        };
    }
    ...
    pub fn deinit(self: *SemanticGraph) void {
        ...
        self.required_capabilities.deinit(self.allocator);
        self.snapshot.deinit();
        self.foreign_grafts.deinit(self.allocator);
    }
    ...
    pub fn requireCapability(self: *SemanticGraph, capability_type: Type) !void {
        ...
        try self.required_capabilities.append(self.allocator, capability_type);
    }
...
const Analyzer = struct {
    ...
    fn addError(self: *Analyzer, message: []const u8, node_id: ?astdb.NodeId) !void {
        ...
        try self.errors.append(self.allocator, SemanticError{
            .message = message,
            .node_id = node_id,
            .cid = null,
        });
...
pub fn analyzeWithASTDB(astdb_system: *astdb.ASTDBSystem, allocator: std.mem.Allocator, profile: Type.Profile) !SemanticGraph {
    var graph = try SemanticGraph.init(allocator, astdb_system, profile);
    ...
    try collectForeignGrafts(&graph);
...
/// Collect foreign graft declarations from use/graft directives in ASTDB units
fn collectForeignGrafts(graph: *SemanticGraph) !void {
    for (graph.astdb_system.units.items) |unit| {
        var idx: usize = 0;
        while (idx < unit.nodes.len) : (idx += 1) {
            const node = unit.nodes[idx];
            if (node.kind != .use_stmt) continue;
            const children = unit.edges[node.child_lo..node.child_hi];
            if (children.len == 0) continue;
            // Look for a string literal child; if none, it's a path import, not a graft
            var has_string = false;
            var string_child_idx: usize = 0;
            for (children, 0..) |cid, i| {
                const cnode = unit.nodes[@intFromEnum(cid)];
                if (cnode.kind == .string_literal) {
                    has_string = true;
                    string_child_idx = i;
                    break;
                }
            ...
            // Determine form: graft alias = origin "module" OR use origin "module"
            var alias_sid_opt: ?astdb.StrId = null;
            var origin_sid: ?astdb.StrId = null;
            if (children.len >= 3 and string_child_idx >= 2) {
                const c0 = unit.nodes[@intFromEnum(children[0])];
                const c1 = unit.nodes[@intFromEnum(children[1])];
                if (c0.kind == .identifier and c1.kind == .identifier) {
                    alias_sid_opt = getIdentifierStrId(unit, c0);
                    origin_sid = getIdentifierStrId(unit, c1);
                }
            }
            if (origin_sid == null and children.len >= 2) {
                const c0 = unit.nodes[@intFromEnum(children[0])];
                if (c0.kind == .identifier) origin_sid = getIdentifierStrId(unit, c0);
            }
            if (origin_sid == null) continue;

            // Extract module string literal (dequote)
            const mod_node = unit.nodes[@intFromEnum(children[string_child_idx])];
            const tok_idx = @intFromEnum(mod_node.first_token);
            if (tok_idx >= unit.tokens.len) continue;
            const tok = unit.tokens[tok_idx];
            const str_sid = tok.str orelse continue;
            var raw = graph.astdb_system.str_interner.getString(str_sid);
            if (raw.len >= 2 and raw[0] == '"' and raw[raw.len - 1] == '"') {
                raw = raw[1 .. raw.len - 1];
            }
            const mod_copy = try graph.allocator.dupe(u8, raw);
            try graph.foreign_grafts.append(graph.allocator, ForeignGraft{
                .alias = alias_sid_opt,
                .origin = origin_sid.?,
                .module = mod_copy,
            });
...
fn collectSymbolTables(graph: *SemanticGraph, allocator: std.mem.Allocator) !void {
    for (graph.astdb_system.units.items) |unit| {
        ...
        while (idx < unit.nodes.len) : (idx += 1) {
            ...
            switch (n.kind) {
                .func_decl => {
                    var params = try std.ArrayList(astdb_core.NodeId).initCapacity(graph.allocator, 0);
                    defer params.deinit(graph.allocator);
                    var param_names = try std.ArrayList(astdb.StrId).initCapacity(graph.allocator, 0);
                    ...
                    for (children) |cid| {
                        ...
                        if (cnode.kind == .identifier and func_name_sid == null) {
                            ...
                            try params.append(graph.allocator, cid);
                            if (getIdentifierStrId(unit, cnode)) |sid| {
                                try param_names.append(graph.allocator, sid);
                            }
                        }
                    }
                ...
                .struct_decl => {
                    var fields = try std.ArrayList(astdb_core.NodeId).initCapacity(graph.allocator, 0);
                    defer fields.deinit(graph.allocator);
                    ...
                    while (i < children.len) : (i += 1) {
                        ...
                        if (cnode.kind == .identifier) {
                            if (!saw_name) {
                                ...
                                try fields.append(graph.allocator, cid);
...
fn analyzeFunctionDecl(analyzer: *Analyzer, node_id: astdb_core.NodeId, node: astdb_core.AstNode, unit: *astdb_core.CompilationUnit) !void {
    ...
    const func_symbol = Symbol{
        ...
        .type = Type.Function,
        .kind = .Function,
        .function_calls = try std.ArrayList(Type.FunctionCallRecord).initCapacity(analyzer.allocator, 0),
        .active_profile = analyzer.graph.active_profile,
        .module_path = null,
...
fn normalizeAndStoreCallArgs(analyzer: *Analyzer, call_id: astdb_core.NodeId, node: astdb_core.AstNode, unit: *astdb_core.CompilationUnit) !void {
    ...
    var args_buf = std.ArrayList(astdb_core.NodeId){};
    defer args_buf.deinit(analyzer.allocator);
    var names_buf = std.ArrayList(?astdb.StrId){};
    ...
    while (i < edges.len) : (i += 1) {
        ...
        if (arg_node.kind == .identifier and i + 1 < edges.len) {
            ...
            try args_buf.append(analyzer.allocator, value_id);
            if (getIdentifierStrId(unit, arg_node)) |sid| {
                try names_buf.append(analyzer.allocator, sid);
            } else try names_buf.append(analyzer.allocator, null);
            i += 1; // consume extra
        } else {
            try args_buf.append(analyzer.allocator, arg_id);
    ...
    var reordered = std.ArrayList(astdb_core.NodeId){};
    defer reordered.deinit(analyzer.allocator);
    ...
    while (pi < pnames_slice.len) : (pi += 1) {
        ...
        while (j < names_buf.items.len) : (j += 1) {
            ...
            if (opt) |aname| {
                if (aname == pname) {
                    try reordered.append(analyzer.allocator, owned[j]);
                    used[j] = true;
                    placed = true;
                    break;
                }
        ...
        if (!placed) {
            ...
            try reordered.append(analyzer.allocator, @as(astdb_core.NodeId, @enumFromInt(0))); // placeholder; will overwrite
        }
    }
...
fn registerStandardLibrary(graph: *SemanticGraph, allocator: std.mem.Allocator) !void {
    ...
    // allocator is now used in the function
}
// Analyze print call with profile-aware arity enforcement
fn analyzePrintCall(analyzer: *Analyzer, node_id: astdb_core.NodeId, node: astdb_core.AstNode, unit: *astdb_core.CompilationUnit) !void {
    const children = unit.edges[node.child_lo..node.child_hi];
    const arg_count = children.len - 1; // subtract callee

    // Get source span for recording
    const tok_idx = @intFromEnum(node.first_token);
    const source_span = if (tok_idx < unit.tokens.len) unit.tokens[tok_idx].span else astdb.Span{ .start = 0, .end = 0 };

    // Profile-specific validation and recording
    switch (analyzer.graph.active_profile) {
        .min => {
            if (arg_count != 1) {
                // Record failure and emit error
                try analyzer.graph.function_calls.append(analyzer.graph.allocator, Type.FunctionCallRecord{
                    .symbol = "print",
                    .profile = .min,
                    .arg_count = arg_count,
                    .source_span = source_span,
                    .status = .invalid_arity,
                });
                try analyzer.addError("print() requires exactly 1 argument in :min profile", node_id);
                return error.SemanticError;
            }
        },
        .s0 => {
            const status = if (arg_count == 1) .ok else .invalid_arity;
            try analyzer.graph.function_calls.append(analyzer.graph.allocator, Type.FunctionCallRecord{
                .symbol = "print",
                .profile = .s0,
                .arg_count = arg_count,
            ...
            if (status == .invalid_arity) {
                try analyzer.addError("print() requires exactly 1 argument in :s0 profile", node_id);
                return error.SemanticError;
            }
        },
        else => {
            // Other profiles: no special handling for now
        },
    }

    // Valid calls: record success
    try analyzer.graph.function_calls.append(analyzer.graph.allocator, Type.FunctionCallRecord{
        .symbol = "print",
        .profile = analyzer.graph.active_profile,
        .arg_count = arg_count,
        .source_span = source_span,
        .status = .ok,
    });
