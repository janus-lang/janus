...
The Janus compiler currently maintains **two separate lexers** that serve different architectural purposes. This document explains why they exist, their differences, and the planned unification strategy.
...
| Error Reporting | Integrates with `DiagnosticCollector` |
...
| Error Reporting | Basic error tracking |
...
| `build.zig` | Module registration (lines 92-97, 121-126) |

## For Future Agents

When adding new token support:
1. **Always update both lexers** for consistency
...
When planning unification:
1. Start with adapter layer to minimize risk
2. Benchmark performance before and after
3. Ensure LSP incremental updates still work
4. Preserve trivia for formatting tools
