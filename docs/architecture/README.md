# Janus Architecture

> "If the foundation is weak, the tower falls."

## Core Components

- **[ASTDB Architecture](ASTDB-ARCHITECTURE.md)**: The immutable query database powering the compiler.
- **[Dispatch Engine](dispatch/dispatch-api-reference.md)**: The heart of Janus polymorphism.
- **[gRPC Backbone](GRPC-BACKBONE.md)**: Communication infrastructure.
- **[Parser Architecture](parser-architecture.md)**: How text becomes thought.
- **[Simdjson Integration](SIMDJSON_INTEGRATION.md)**: High-performance JSON parsing.
