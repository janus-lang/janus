...
// Integration Test: Panic End-to-End
//
...
test "Runtime Panic Execution" {
    ...
    var emitter = try qtjir.llvm_emitter.LLVMEmitter.init(allocator, "panic_test");
    ...
    const exe_file_path = try std.fs.path.join(allocator, &[_][]const u8{ ir_path, "panic_test_exe" });
