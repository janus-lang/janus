...
// Test the granite interner with ASTDB types to isolate the issue
const ids = @import("compiler/libjanus/astdb/ids.zig");
const StrId = ids.StrId;

pub const ASTDBMinimalInterner = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, capacity: u32) !ASTDBMinimalInterner {
        const strings = try allocator.alloc([]const u8, capacity);
        const arena = std.heap.ArenaAllocator.init(allocator);

        return ASTDBMinimalInterner{
    ...
    pub fn deinit(self: *ASTDBMinimalInterner) void {
        self.arena.deinit();
        self.parent_allocator.free(self.strings);
        self.* = undefined;
    }

    pub fn get(self: *ASTDBMinimalInterner, s: []const u8) !StrId {
    ...
    pub fn str(self: *const ASTDBMinimalInterner, id: StrId) []const u8 {
    ...
    pub fn count(self: *const ASTDBMinimalInterner) u32 {
        return self.string_count;
    }
};

test "ASTDB Minimal Granite Interner - Basic Test" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    const allocator = gpa.allocator();

    {
        var interner = try ASTDBMinimalInterner.init(allocator, 10);
