...
pub const LLVMCodegen = struct {
    ...
    pub fn emit(module: *IR.Module, out_path: []const u8, allocator: std.mem.Allocator) !void {
        ...
        try cg.writeHeader(writer);
        try cg.emitGlobals(writer, module);
        try cg.emitFunctions(writer, module);
        try writer.flush();
    }
        ...
        for (module.instructions.items) |instr| {
            if (instr.kind == .StringConst) {
                if (instr.result) |v| {
                    ...
                    try w.print("@s{d} = private unnamed_addr constant [{d} x i8] c", .{ v.id, esc.len });
                    try w.writeAll(esc);
                    try w.print(", align 1\n", .{});
                }
            }
    ...
    fn escapeForLLVM(self: *LLVMCodegen, raw: []const u8) ![]u8 {
        ...
        for (src) |b| {
            ...
            if (b == '\\' or b == '"') {
                ...
                try out.appendSlice(self.allocator, "\\0A");
            } else if (b == 0x09) {
                try out.appendSlice(self.allocator, "\\09");
            } else {
                if (b >= 0x20 and b <= 0x7E) {
                    ...
                    try out.append(self.allocator, hch);
                    try out.append(self.allocator, lch);
                }
            }
        }
        // zero-terminate
        try out.appendSlice(self.allocator, "\\00");
        try out.append(self.allocator, '"');
