...
# Janus Language Grammar Specification

The **Janus Surface Grammar v1.0.0 (Immutable Lock-in)** — hardened for a decade of evolution. This specification prioritizes **comprehension over keystrokes** and **predictability over flexibility**, based on cognitive science and human factors engineering.
...
- **Zero-arg call rule** enforced by `call_suffix`: no args and no block ⇒ not a call
- Named args use `label:` exactly once per label; order is source order
- `and/or/not` are first-class; `&&/||/!` kept for folks who like symbols
